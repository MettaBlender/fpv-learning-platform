                    ...
                    {droneComponents[componentType]
                      ?.filter((component) => {
                        ...
                        return Object.entries(filters).every(([filterKey, filterValues]) => {
                          // Wenn keine Werte für diesen FilterKey ausgewählt sind, filtert er nichts
                          if (filterValues.length === 0) {
                            return true
                          }

                          // Wenn die Komponente keine Optionen hat, wird sie von diesem Filter nicht betroffen
                          if (!component.options || component.options.length === 0) {
                            return true
                          }

                          // Überprüfen, ob eine der Optionen der Komponente mit einem der ausgewählten Filterwerte übereinstimmt
                          // Eine Komponente besteht diesen Filter, wenn MINDESTENS EINE ihrer Optionen
                          // den filterKey enthält UND ihr Wert unter den ausgewählten filterValues ist.
